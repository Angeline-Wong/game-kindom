...
export function WeatherEffects({ weather, minuteOfDay = 720, suppressDaylight = false, suppressWeather = false }: { weather: WeatherKind; minuteOfDay?: number; suppressDaylight?: boolean; suppressWeather?: boolean }) {
  const mode = weather === '雨' ? 'rain' : weather === '雪' ? 'snow' : weather === '晴' ? 'sunny' : 'fog';
  const phase = getTimePhase(minuteOfDay);
  const label = weather === '雪' ? '雪景特效' : weather === '雨' ? '雨景特效' : weather === '晴' ? '晴景特效' : '雾景特效';
  return <>{!suppressDaylight && <div className={`daylight-overlay daylight-${phase}`} aria-label={`${phase === 'morning' ? '晨光' : phase === 'noon' ? '日光' : phase === 'evening' ? '暮色' : '月夜'}氛围`} aria-hidden="true">{phase === 'night' && <><i className="night-moon" /><i className="night-haze" /></>}</div>}{!suppressWeather && <div className={`weather-overlay weather-${mode}`} aria-label={label} aria-hidden="true">
