...
export function RoyalMarriagePanel({ state, onChange, onClose, onNotice, onMarriageComplete }: { state: GameState; onChange: (state: GameState) => void; onClose: () => void; onNotice: (message: string) => void; onMarriageComplete: (royalId: string, message: string) => void }) {
  ...
  const finishCandidate = (candidate: MarriageCandidate) => {
    if (!royal) return;
    const message = `${royal.name}与${candidateLabel(candidate)}${candidate.name}的婚配已定，礼部将择吉完婚。`;
    onChange(marryCandidate(state, royal.id, candidate));
    onMarriageComplete(royal.id, message);
  };
  ...
  const chooseMinister = (ministerId: string) => {
    ...
    const message = `${royal.name}与${minister.rank ?? ''}${minister.office ?? minister.title}${minister.name}的婚配已定。`;
  ...
  return <div className="interaction-overlay royal-marriage-overlay" role="dialog" aria-modal="true" aria-label="皇嗣婚配">
    <section className="royal-marriage-panel">
      <header><div><small>撷芳殿宫务</small><h2>皇嗣婚配</h2></div><button type="button" aria-label="关闭皇嗣婚配" onClick={onClose}><X /></button></header>
      ...
      {conflictMinisterId && royal && <div className="marriage-conflict"><section><h3>婚配有碍</h3><p>{state.people[conflictMinisterId].name}已有原配。可换一位大臣，或刺死原配后继续婚配。</p><button onClick={() => setConflictMinisterId(null)}>换一位</button><button className="danger" onClick={() => { const minister = state.people[conflictMinisterId]; const message = `${minister.name}原配已被刺死，${royal.name}与${minister.name}奉旨成婚。`; onChange(marryMinister(state, royal.id, conflictMinisterId, true)); onMarriageComplete(royal.id, message); }}>刺死原配</button></section></div>}
    </section>
  </div>;
}
