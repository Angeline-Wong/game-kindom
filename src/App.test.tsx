...
import App, { AttendantEventDialog, ConsortAwayDialog, RoyalNameDialog } from './App';
import { vi } from 'vitest';
import type { GameEvent, PersonRecord } from './game/gameState';

describe('App shell', () => {
  it('asks for each royal name using the child title after the birth announcement', () => {
    const child = {
      id: 'child-1', kind: 'PRINCE', name: '待赐名皇子一', named: false, sex: 'MALE',
      birthDate: { year: 3, month: 10, day: 18 }, age: 0, title: '皇长子', residence: '撷芳殿', sceneId: 'xiefang', status: 'NORMAL',
      assets: { avatar: 'portrait.prince', portrait: 'portrait.prince' }, parents: ['emperor'], children: [], stats: {}, traits: [],
    } as unknown as PersonRecord;
    render(<RoyalNameDialog child={child} people={{ emperor: child, [child.id]: child }} usedNames={[]} onConfirm={() => false} />);

    expect(screen.getByText('请您给皇长子赐名。')).toBeInTheDocument();
    expect(screen.queryByText(/礼部已为新生/)).not.toBeInTheDocument();
  });
