...
import { getDemotionOptions, getNextPromotionOption, getPromotionOptions, isConsortRankAvailable } from './ranks';
...
describe('rank choices', () => {
  ...
  it('blocks the next promotion when its capped rank is full', () => {
    expect(getNextPromotionOption('贵人', { 嫔: 6 })).toMatchObject({ label: '嫔', available: false });
    expect(isConsortRankAvailable('妃', { 妃: 3 })).toBe(true);
    expect(isConsortRankAvailable('妃', { 妃: 4 })).toBe(false);
  });
