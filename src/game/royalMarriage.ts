...
import { completeRoyalMarriage } from './royalRoster';
...
export function marriageTitleFollowUp(state: GameState, royalId: string) {
  const royal = state.people[royalId];
  if (!royal || (royal.kind !== 'PRINCE' && royal.kind !== 'PRINCESS') || royal.royalTitle) return null;
  return { personId: royal.id, kind: royal.kind } as const;
}
...
export function createMinistryCandidates(state: GameState, royalId: string) {
  ...
  const ministers = Object.values(state.people).filter((person) =>
    person.kind === 'MINISTER' && isPersonAlive(person));
...
function createCandidatePerson(candidate: MarriageCandidate, royal: PersonRecord, date: GameDate): PersonRecord {
  ...
  return {
    ...
    title: royal.kind === 'PRINCESS' ? '驸马' : '王妃',
    royalSpouseOf: royal.id,
    residence: '宫外',
    residenceType: 'OUTSIDE_PALACE',
    isSummoned: false,
    sceneId: 'outside',
    status: 'NORMAL',
    ...
    children: [],
    stats: { 魅力: candidate.charm, 谋略: candidate.strategy, 野心: 25 + Math.floor((candidate.strategy + candidate.charm) / 4) },
...
export function marryCandidate(
  ...
  const royal = state.people[royalId];
  if (!isPersonAlive(royal) || hasSpouse(state, royalId)) return state;
...
export function marryMinister(
  ...
  if (!isPersonAlive(royal) || !isPersonAlive(minister) || hasSpouse(state, royalId)) return state;
  ...
  if (existingSpouse) {
    state = killPerson(state, existingSpouse.id, '婚配刺杀');
    Object.assign(people, state.people);
    history.push(...state.history.filter(entry => !history.some(h => h.id === entry.id)));
  ...
  return completeRoyalMarriage({ ...state, people, relationships, history }, royalId);
}

export function scheduleMarriageBanquet(state: GameState, royalId: string): GameState {
  const royal = state.people[royalId];
  if (!isPersonAlive(royal) || !eligibleRoyalHeirs(state).some((person) => person.id === royalId)) return state;
...
function motherOf(state: GameState, royal: PersonRecord) {
  return royal.parents.map((id) => state.people[id]).find((person) => person?.kind === 'CONSORT' && isPersonAlive(person));
}
...
export function processRoyalMarriagesForDay(state: GameState, date: GameDate): GameState {
  ...
  next.royalMarriages = next.royalMarriages.map((record) => {
    ...
    if (!isPersonAlive(royal)) return ['COMPLETED', 'REJECTED'].includes(record.status) ? record : { ...record, status: 'REJECTED' };
    if (!record.banquetOn) return record;
...
export function resolveRoyalMarriageEvent(state: GameState, eventId: string, choiceId: string): GameState {
  ...
  if (!event || event.personIds.some(id => !isPersonAlive(state.people[id])) || (event.type !== 'MARRIAGE_BANQUET' && event.type !== 'MARRIAGE_NOTICE')) return state;
  let next: GameState = {
    ...state,
  ...
  if (choiceId === 'approve-marriage' && candidate) {
    next = marryCandidate(next, record.royalId, candidate);
    next = completeRoyalMarriage(next, record.royalId);
  }
...
export function applyRoyalBanquetLocations(state: GameState, date: GameDate, minuteOfDay: number): GameState {
  ...
  if (!isPersonAlive(royal)) return state;
