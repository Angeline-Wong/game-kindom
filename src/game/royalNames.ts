...
/** Compact the formal title for the limited-width heir roster badge. */
export function royalTitleDisplay(title: string | undefined) {
...
export function royalBirthIdentity(person: Pick<PersonRecord, 'kind' | 'birthOrder' | 'title' | 'preHeirTitle'>) {
  if (person.kind !== 'PRINCE' && person.kind !== 'PRINCESS') return person.title;
  const order = person.birthOrder;
  if (Number.isInteger(order) && order! > 0) {
    const suffix = person.kind === 'PRINCE' ? '子' : '女';
    const prefix = order === 1 ? '长' : order === 2 ? '次' : toChineseNumber(order!);
    return `皇${prefix}${suffix}`;
  }
  return person.preHeirTitle ?? (/皇子$|公主$/.test(person.title) ? person.title : person.title);
}

export function royalIdentityTitle(person: Pick<PersonRecord, 'kind' | 'title' | 'rank' | 'preHeirTitle'>) {
  const current = person.rank ?? person.title;
  if (person.kind !== 'PRINCE' && person.kind !== 'PRINCESS') return current;
  const sequence = person.preHeirTitle ?? (/皇子$|公主$/.test(person.title) ? person.title : undefined);
  return sequence && sequence !== current ? `${sequence} · ${current}` : current;
}
