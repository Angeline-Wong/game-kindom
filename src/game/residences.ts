...
export function residenceStateFromPeople(people: Record<string, { id: string; kind: string; residence?: string }>): ResidenceState {
  const occupantByRoom = new Map<string, string>();
  Object.values(people).forEach((person) => {
    if (person.kind !== 'CONSORT') return;
...
export function getAvailableMainHalls(people: Record<string, { id: string; kind: string; residence?: string }>) {
  const occupied = new Set(Object.values(people).filter((person) => person.kind === 'CONSORT').map((person) => person.residence).filter(Boolean));
